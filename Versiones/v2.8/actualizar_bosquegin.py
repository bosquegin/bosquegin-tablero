#!/usr/bin/env python3
"""
actualizar_bosquegin.py  —  v2.4  (2026-05-20)
...
def update_consolidado():
    ...
    for file_date, fpath in new_files:
        ...
        # Encontrar fila de cabeceras: primero en las 5 primeras filas (formato clasico),
        # si no se encuentra y es archivo Comp, buscar en todo el archivo
        # (GC exporta primero el resumen total y luego la seccion por deposito)
        def _is_dep_hdr(r):
            return r and r[0] and str(r[0]).strip().upper() in ("DEPOSITO", "DEP\xd3SITO", "DEP\xd3SITO", "DEPÓSITO")
...
def fetch_costos_completo():
    """
    Lee la hoja COSTOS desde el CSV local (exportado de Google Sheets).
    Archivo: Data/Costos y PVP/Analisis de costos y PVP - COSTOS.csv
    Estructura gviz:
      fila 0 = índices internos (-2,-1,0,1,...)
      fila 1 = etiquetas de año/fecha
      fila 2 = etiquetas de mes
    ...
    # ── Intentar leer CSV local, luego Google Sheets ─────────────────────────
    all_rows = None
    source   = "local"
    ...
    # ── Mapear columnas desde la fila 3 de headers ───────────────────────────
    hdrs   = all_rows[3]
    row1   = all_rows[1]  # etiquetas de año
    row2   = all_rows[2]  # etiquetas de mes

    # Columnas base de cada bloque mensual (COSTO FINAL, MB=+2, PVP=+8)
    costo_cols = [i for i, h in enumerate(hdrs) if h.strip() == "COSTO FINAL"]

    # Bloques RESUMEN: buscar en row2 los que dicen "RESUMEN ..."
    resumen_pos = {}   # {"2025": col_idx, "2026": col_idx, ...}
    for i, r2 in enumerate(row2):
        if "RESUMEN" in r2.upper():
            yr_match = r2.strip().split()[-1]  # ej. "RESUMEN 2025" → "2025"
    ...
    for row in all_rows[4:]:
        ...
        # Costo más reciente: escanear costo_cols de derecha a izquierda
...
def main():
    ...
    print("\n[4c/5] Leyendo Remitos GC (Data/Salidas/GC/)...")
    try:
        _, ventas_gc_hasta, monthly_gc = parse_remitos_gc(prod_lookup=inv_gen, costos=costos)
        if monthly_gc:
            # GC overrides salidas for matching (yr, mo)
            merged = False
            for yr, yr_mo in monthly_gc.items():
                if yr not in monthly_raw:
                    monthly_raw[yr] = {}
                for mo_s, mo_data in yr_mo.items():
                    monthly_raw[yr][mo_s] = mo_data
                    merged = True
            if merged:
                ventas, ventas_hasta, monthly_raw = _build_ventas_output(monthly_raw)
                print("  -> Datos GC integrados; ahora hasta %s" % ventas_hasta)
    except Exception as e:
        print("  Advertencia Remitos GC: %s" % e)
