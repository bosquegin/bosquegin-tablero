...
_429_BACKOFFS_S = (10, 30, 60)  # espera creciente antes de reintentar un 429


def _api_get(path, params=None, retry_on_401=True, _retries_429=0):
    ...
    try:
        ...
        if e.code == 401 and retry_on_401:
            _fetch_new_token()
            return _api_get(path, params, retry_on_401=False, _retries_429=_retries_429)
        # 429 real detectado 2026-08-07: a veces es un tope breve (ráfaga de
        # llamadas) que se libera en segundos/minutos, no siempre un tope
        # diario. Reintentar con espera creciente antes de rendirse evita
        # perder toda la corrida por un pico momentáneo.
        if e.code == 429 and _retries_429 < len(_429_BACKOFFS_S):
            wait_s = _429_BACKOFFS_S[_retries_429]
            print(f"    [Contabilium] 429, reintentando en {wait_s}s "
                  f"({_retries_429+1}/{len(_429_BACKOFFS_S)})...")
            time.sleep(wait_s)
            return _api_get(path, params, retry_on_401=retry_on_401, _retries_429=_retries_429 + 1)
        raise
...
def search_comprobantes(fecha_desde, fecha_hasta):
    """
    Todos los comprobantes (facturas, cotizaciones, notas de crédito, etc.)
    emitidos entre fecha_desde y fecha_hasta (strings 'YYYY-MM-DD'), paginando
    automáticamente. Cada item NO trae el detalle de líneas (Items=None) —
    para eso usar get_comprobante(id). Devuelve lista de dicts.
    """
    out = []
    page = 1
    while True:
        data = _api_get("/comprobantes/search",
                        {"fechaDesde": fecha_desde, "fechaHasta": fecha_hasta, "page": page})
        items = data.get("Items", [])
        if not items:
            break
        out.extend(items)
...
def get_comprobante(id_comprobante):
    """Detalle completo de un comprobante (incluye Items: líneas con Codigo/Cantidad,
    e Inventario: id del depósito del que descuenta stock)."""
    return _api_get("/comprobantes/getById", {"id": id_comprobante})
