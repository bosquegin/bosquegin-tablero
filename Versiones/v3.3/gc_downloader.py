...
# ── Remitos ───────────────────────────────────────────────────────────────────
def _html_to_remitos_rows(html: str) -> list:
    """Parsea el HTML de tabla que devuelve GC y retorna lista de tuplas con las
    mismas columnas que el Excel: (Informe, Remito, Nro.Pedido, Fecha, Fantasía,
    RazonSocial, Vendedor, Usuario, Tipo, Cantidad, Barril, Lote, LitrosBrl,
    LitrosEnv, Producto, Estilo, Codigo, Proveedor, Deposito).
    La primera TR es la fila de encabezado (ColumnHeaderStyle) — se descarta.
    La columna Fecha (índice 3) se convierte a datetime para compatibilidad con openpyxl."""
    import html as _html_mod
    from datetime import datetime as _dt

    def _cell(td):
        t = re.sub(r'<[^>]+>', '', td).strip()
        return _html_mod.unescape(t) if t and t != '&nbsp;' else None

    def _to_datetime(s):
        """Convierte "D/M/YYYY H:MM:SS" → datetime. Fallback: string original."""
        if not s:
            return None
    ...
    for tr in re.finditer(r'<TR class=["\']replace[12]["\']>(.*?)</TR>', html, re.DOTALL | re.IGNORECASE):
        inner = tr.group(1)
        # Detectar fila de encabezado por ColumnHeaderStyle o etiqueta <b>
        if not header_skipped and ('ColumnHeaderStyle' in inner or '<b>' in inner):
            header_skipped = True
            continue
        tds = re.findall(r'<TD[^>]*>(.*?)</TD>', inner, re.DOTALL | re.IGNORECASE)
        if len(tds) < 19:
            continue
        cells = [_cell(td) for td in tds[:19]]
        cells[3] = _to_datetime(cells[3])   # col 3 = Fecha → datetime
...
def _fetch_remitos(cookie_str: str, fecha_desde_date=None, verbose: bool = True) -> str | None:
    ...
    GC devuelve una tabla HTML que se parsea directamente (ya no envía Excel+JSON).
    ...
    hdrs = {
        **_base_headers(cookie_str),
        "Content-Type":     "application/x-www-form-urlencoded",
        "X-Requested-With": "XMLHttpRequest",
    }
    body = urllib.parse.urlencode({
        "filtro": "", "tieneGrafico": "false", "tieneRanking": "false",
        "id": "", "entidad": "",
    ...
    rows = _html_to_remitos_rows(html)
    if not rows:
        raise RuntimeError(f"GC Remitos: 0 filas parseadas del HTML ({len(html)} bytes)")

    # Escribir Excel con la misma estructura que el archivo anterior
    HEADERS = ["Informe","Remito","Nro. Pedido","Fecha","Nombre de fantasía",
               "Razón social","Vendedor","Usuario","Tipo","Cantidad",
               "Barril/Envase","Lote","Litros Barril","Litros envase",
               "Producto","Estilo","Código","Proveedor de logística","Depósito"]

    wb  = _xl.Workbook(write_only=True)
    ...
    # Eliminar remitos anteriores — solo debe existir uno a la vez
    ...
    wb.save(out)
    if verbose: print(f"  [GC] OK Remitos -> {rem_name}  ({len(rows)} filas)")
    return out
