...
# Se usa el mismo criterio en todo el tablero para el desglose semanal:
# cada mes se parte en semanas de 7 días fijas (1-7, 8-14, 15-21, 22-28, 29-fin),
# en vez de la semana ISO (que puede cruzar meses/años y no calza con "semana
# del mes" como lo entiende el usuario).
def _semana_greg(d):
    """Clave de semana calendario 'YYYY-MM-S#' para una fecha (date)."""
    return f"{d.year}-{d.month:02d}-S{(d.day - 1) // 7 + 1}"


def _semana_greg_cierre(sem_key):
    """Último día cubierto por una semana 'YYYY-MM-S#'."""
    y, mo, s = int(sem_key[:4]), int(sem_key[5:7]), int(sem_key[9:])
    ultimo_dia_mes = calendar.monthrange(y, mo)[1]
    dia = min(s * 7, ultimo_dia_mes)
    return date(y, mo, dia)


def _semana_greg_siguiente(sem_key):
    """Semana 'YYYY-MM-S#' inmediatamente posterior a sem_key."""
    y, mo, s = int(sem_key[:4]), int(sem_key[5:7]), int(sem_key[9:])
    ultimo_dia_mes = calendar.monthrange(y, mo)[1]
    max_s = (ultimo_dia_mes - 1) // 7 + 1
    if s < max_s:
        return f"{y}-{mo:02d}-S{s + 1}"
    if mo == 12:
        y, mo = y + 1, 1
    else:
        mo += 1
    return f"{y}-{mo:02d}-S1"
    ...
    for yr in sorted(monthly.keys()):
        ...
        for mo_s, mo_data in yr_mo.items():
            ...
            md_yr["weekly"][mo_s] = sorted(
                [{"w": wk,
                  "label": f"Semana {wk} · {_WK_RANGES.get(wk, wk)} {mes_nom}",
        ...
        for cl in cl_yr.values():
            ...
            cl["monthly"] = [
                {"mes": mo, "label": MESES_NOM[mo],
                 ...
                 "weekly": sorted(
                     [{"w": wk,
                       "label": f"Semana {wk} · {_WK_RANGES2.get(wk, wk)} {MESES_NOM[mo]}",
