...
//  para el sitio publicado: en vez de users.json (local, nunca se sube al
//  repo) lee y escribe directo auth_static.js en GitHub — el mismo archivo
//  público del que ya lee verifyToken()/fetchUsers() para validar sesiones.
//  Hashing PBKDF2-SHA256/100000 idéntico al de _hash_pw()/_check_pw() en
//  Python y al de _bgVerifyPw() del login client-side, para que una
//  contraseña creada acá funcione para loguearse y viceversa.
//
//  Nota: esto trata auth_static.js como fuente de verdad del lado cloud,
//  separada de users.json del lado local — no se sincronizan solas entre
//  sí. Si se edita un usuario desde el tablero local Y desde el publicado
//  sin correr Actualizar (que sube users.json -> auth_static.js) entre
//  medio, el que se guarde último pisa al otro. Mismo patrón de
//  "reaplicar sobre lo último publicado" ya usado en guardarProyeccionEdit.
// ═══════════════════════════════════════════════════════════════════════

const AUTH_STATIC_PATH = "auth_static.js";
const ALL_SECTIONS = [
...
// que crypto.subtle.deriveBits(...) en _bgVerifyPw() del login.
...
// Mismo formato que genera generate_auth_static() en Python — un objeto
// JSON por línea dentro de "window.BG_AUTH = [...]" — para que un
// Actualizar/edición posterior desde cualquiera de los dos lados lo
// pueda seguir leyendo igual.
function serializeAuthStatic(users) {
  const entries = users.map((u) => JSON.stringify({
    username: u.username,
    ...
    salt: u.salt,
    cloud_token: u.cloud_token || "",
  }));
  return "// Auto-generado — no editar manualmente\nwindow.BG_AUTH = [\n  "
    + entries.join(",\n  ") + "\n];\n";
}

function parseAuthStatic(text) {
  const m = text.match(/window\.BG_AUTH\s*=\s*(\[[\s\S]*?\])\s*;/);
  if (!m) return [];
  try {
    return JSON.parse(m[1]);
  } catch (e) {
...
async function handleAdminUsers(request, env, origin) {
  ...
  const actor = await verifyToken(actorUsername, actorToken);
  ...
  for (let intento = 0; intento < 2; intento++) {
    const file = await ghGetFile(env, AUTH_STATIC_PATH);
    let users = file ? parseAuthStatic(file.content) : [];
    ...
    if (action === "list") {
      ...
      return json({ ok: true, users: safe }, 200, origin);
    }
    ...
    if (action === "create") {
      ...
      users.push(newUser);
      try {
        await ghPutFile(env, AUTH_STATIC_PATH, serializeAuthStatic(users),
          file ? file.sha : null, `admin: crear usuario ${newUsername}`);
        const { password_hash, salt: _s, ...safe } = newUser;
        return json({ ok: true, user: { ...safe, id: newUser.username } }, 200, origin);
      } catch (e) {
    ...
    if (action === "update") {
      ...
      if (idx === -1) return json({ ok: false, error: "usuario no encontrado" }, 404, origin);
      const u = users[idx];
      ...
      try {
        await ghPutFile(env, AUTH_STATIC_PATH, serializeAuthStatic(users),
          file.sha, `admin: editar usuario ${uid}`);
        const { password_hash, salt: _s, ...safe } = u;
        return json({ ok: true, user: { ...safe, id: u.username } }, 200, origin);
      } catch (e) {
        if (e.status === 409 && intento === 0) continue;
        throw e;
      }
    ...
    if (action === "delete") {
      ...
      if (uid === actor.username) {
        return json({ ok: false, error: "No podés borrar tu propio usuario" }, 400, origin);
      }
      const orig = users.length;
      users = users.filter((u) => u.username !== uid);
      if (users.length === orig) return json({ ok: false, error: "usuario no encontrado" }, 404, origin);
      try {
        await ghPutFile(env, AUTH_STATIC_PATH, serializeAuthStatic(users),
          file.sha, `admin: borrar usuario ${uid}`);
