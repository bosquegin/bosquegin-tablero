 ...
 * POST /login          → verifica username+password server-side contra
 *                      AUTH_KV (Workers KV) y devuelve el usuario (con
 *                      cloud_token) si es correcto. El cliente ya no
 *                      necesita ni recibe password_hash/salt de nadie.
 * POST /admin_users    → CRUD de usuarios (list/create/update/delete),
 *                      contra AUTH_KV.
 *
 * Usuarios (password_hash/salt/cloud_token) viven en AUTH_KV, NO en el
 * repo git — el repo es público y cualquier archivo en él, esté o no
 * enlazado desde el sitio, se puede descargar. Ver el comentario más
 * abajo, arriba de kvKey().
 */
...
// ═══════════════════════════════════════════════════════════════════════
//  Usuarios — Cloudflare Workers KV (binding AUTH_KV), NO en el repo git.
//  El repo (bosquegin-tablero) es público: cualquier archivo, esté o no
//  referenciado desde el sitio, se puede descargar vía la Contents API o
//  raw.githubusercontent.com aunque no tenga link visible. Guardar
//  password_hash/salt/cloud_token en un archivo del repo (como se hacía
//  antes en auth_static.js) los expone sin importar el nombre o si el
//  archivo "no se usa" -- por eso viven acá, en KV, que el Worker puede
//  leer/escribir pero nadie puede descargar desde afuera.
// ═══════════════════════════════════════════════════════════════════════
...
async function kvGetUser(env, username) {
  if (!username) return null;
  const raw = await env.AUTH_KV.get(kvKey(username));
  if (!raw) return null;
  try {
...
async function kvPutUser(env, user) {
  await env.AUTH_KV.put(kvKey(user.username), JSON.stringify(user));
}

async function kvDeleteUser(env, username) {
  await env.AUTH_KV.delete(kvKey(username));
}
...
async function kvListUsers(env) {
  ...
  do {
    ...
    users.push(...chunk.filter(Boolean));
...
async function verifyToken(env, username, token) {
  ...
  const u = await kvGetUser(env, username);
  if (u && timingSafeEqual(u.cloud_token || "", token)) return u;
  return null;
}
...
async function handleGuardarAbastecimiento(request, env, origin) {
  ...
  const user = await verifyToken(env, username, token);
...
// ═══════════════════════════════════════════════════════════════════════
//  Administración de usuarios — mismo CRUD que servidor_bosquegin.py
//  (_admin_create_user / _admin_update_user / _admin_delete_user), pero
//  para el sitio publicado: lee y escribe en AUTH_KV en vez de users.json.
//  Hashing PBKDF2-SHA256/100000 idéntico al de _hash_pw()/_check_pw() en
//  Python y al del login server-side (handleLogin), para que una
//  contraseña creada acá funcione para loguearse y viceversa.
//
//  Nota: KV (lado cloud) y users.json (lado local) son dos fuentes de
//  verdad separadas — no se sincronizan solas. servidor_bosquegin.py
//  espeja cada create/update/delete local hacia acá (mismo endpoint,
//  best-effort) para que no se desincronicen en el uso normal.
// ═══════════════════════════════════════════════════════════════════════

const ALL_SECTIONS = [
  "overview", "inventario", "inv-intel", "ventas", "destileria",
  "supply", "proyeccion", "logistica", "calendario", "costos", "inv-insumos",
  "control-inventario",
];
...
// Debe dar EXACTAMENTE el mismo resultado que hashlib.pbkdf2_hmac("sha256",
// password, salt, 100_000) en Python (32 bytes / 256 bits por defecto) y
// que crypto.subtle.deriveBits(...) del lado local.
...
function safeUser(u) {
  ...
  return { ...rest, id: u.username };
}

// Login server-side: el cliente ya no recibe hash/salt de nadie — solo
// posta username+password acá y, si es correcto, se lleva el objeto de
// sesión (con cloud_token, necesario para las demás llamadas). Mensaje de
// error genérico en ambos casos (usuario inexistente o contraseña
...
async function handleLogin(request, env, origin) {
  ...
  const GENERIC_ERR = "Usuario o contraseña incorrectos";
  if (!username || !password) return json({ ok: false, error: GENERIC_ERR }, 401, origin);

  const u = await kvGetUser(env, username);
  if (!u) return json({ ok: false, error: GENERIC_ERR }, 401, origin);

  const hash = await pbkdf2Hash(password, u.salt);
  if (!timingSafeEqual(hash, u.password_hash)) {
    return json({ ok: false, error: GENERIC_ERR }, 401, origin);
  }

  return json({
    ok: true,
    user: {
      username: u.username, nombre: u.nombre, role: u.role,
      sections: u.sections, cloud_token: u.cloud_token,
    },
  }, 200, origin);
}
...
async function handleAdminUsers(request, env, origin) {
  ...
  // bulk_import es la migración única users.json (local) -> AUTH_KV. La
  // primera vez que corre, KV está vacío y todavía no hay ningún admin con
  // el que autenticarse acá adentro -- por eso se permite SOLO mientras KV
  // no tenga ni un usuario todavía (ventana que se cierra sola apenas se
  // crea el primero). Una vez migrado, esta acción vuelve a pedir admin
  // como cualquier otra (por si hace falta re-importar algo puntual).
  if (action === "bulk_import") {
    const existentes = await kvListUsers(env);
    if (existentes.length > 0) {
      const actorUsername = (body.username || "").trim();
      const actorToken = (body.cloud_token || "").trim();
      const actor = await verifyToken(env, actorUsername, actorToken);
    ...
    for (const u of incoming) {
      if (!u.username || !u.password_hash || !u.salt) continue;
      // Preserva password_hash/salt (nadie pierde su contraseña) pero
      // SIEMPRE rota cloud_token -- los que había hasta ahora circularon
      // en texto plano en auth_static.js público, así que quedan
      // invalidados de una.
      await kvPutUser(env, {
        username: u.username,
        nombre: u.nombre || u.username,
        role: u.role || "viewer",
        sections: u.sections || ALL_SECTIONS,
        password_hash: u.password_hash,
        salt: u.salt,
        cloud_token: randomHex(32),
      });
      count++;
    }
    return json({ ok: true, imported: count }, 200, origin);
  }

  const actorUsername = (body.username || "").trim();
  const actorToken = (body.cloud_token || "").trim();
  const actor = await verifyToken(env, actorUsername, actorToken);
  if (!actor) return json({ ok: false, error: "No autorizado" }, 401, origin);
  if (actor.role !== "admin") return json({ ok: false, error: "Sin permiso" }, 403, origin);

  if (action === "list") {
    const users = await kvListUsers(env);
    return json({ ok: true, users: users.map(safeUser) }, 200, origin);
  }

  if (action === "create") {
    const newUsername = String(body.new_username || "").trim();
    if (!newUsername) return json({ ok: false, error: "username requerido" }, 400, origin);
    if (await kvGetUser(env, newUsername)) {
      return json({ ok: false, error: "usuario ya existe" }, 409, origin);
    }
    const pw = String(body.password || "");
    if (!pw) return json({ ok: false, error: "contraseña requerida" }, 400, origin);
    const salt = randomHex(16);
    let sections = Array.isArray(body.sections) ? body.sections : ALL_SECTIONS;
    const role = body.role || "viewer";
    if (role === "admin" && !sections.includes("admin")) sections = sections.concat(["admin"]);
    const newUser = {
      username: newUsername, nombre: body.nombre || newUsername,
      password_hash: await pbkdf2Hash(pw, salt), salt,
      role, sections, cloud_token: randomHex(32),
    };
    await kvPutUser(env, newUser);
    return json({ ok: true, user: safeUser(newUser) }, 200, origin);
  }

  if (action === "update") {
    const uid = String(body.uid || "");
    const u = await kvGetUser(env, uid);
    if (!u) return json({ ok: false, error: "usuario no encontrado" }, 404, origin);
    ...
    if (body.password) {
      const salt = randomHex(16);
      u.password_hash = await pbkdf2Hash(String(body.password), salt);
      u.salt = salt;
    }
    await kvPutUser(env, u);
    return json({ ok: true, user: safeUser(u) }, 200, origin);
  }

  if (action === "delete") {
    const uid = String(body.uid || "");
    // Chequeo extra que servidor_bosquegin.py no tiene (ahí solo lo evita
    // la UI ocultando el botón) -- inofensivo, protege también si alguien
    // llama la API directo.
    if (uid.toLowerCase() === actor.username.toLowerCase()) {
      return json({ ok: false, error: "No podés borrar tu propio usuario" }, 400, origin);
    }
    const existing = await kvGetUser(env, uid);
    if (!existing) return json({ ok: false, error: "usuario no encontrado" }, 404, origin);
    await kvDeleteUser(env, uid);
    return json({ ok: true }, 200, origin);
  }

  return json({ ok: false, error: "Acción desconocida" }, 400, origin);
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/admin_users" && request.method === "POST") {
        return await handleAdminUsers(request, env, origin);
      }
